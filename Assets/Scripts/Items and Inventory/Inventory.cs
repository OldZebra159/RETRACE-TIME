...
public class Inventory : MonoBehaviour , ISaveManager
{
    ...
    private float lastTimeUsedAromr;
    ...
    public bool CanUseAromr()
    {
        ItemData_Equipment currentArmor = GetEquipment(EquipmentType.Armor);

        if (Time.time > lastTimeUsedAromr + armorCooldown)
        {
            armorCooldown = currentArmor.itemCooldown;
            lastTimeUsedAromr = Time.time;
            return true;
        }

        Debug.Log("Aromr on cooldown");
        return false;
    }
